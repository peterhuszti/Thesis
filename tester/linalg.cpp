...
qr givens(const sparse_matrix &mtx)
{
    ...
    while(!kesz)
    {
        ...
        {
            setRCS(r,c,s,R.at(chosen.jj,chosen.jj),R.at(chosen.ii,chosen.jj));
                        //r,c,s kiszámítása
...
void setRCS(double &r,double &c, double &s,double a, double b)
{
    if(b==0)
    {
        c=copysign(1,a);
...
bool equals(const std::vector<std::vector<double> > &a, const std::vector<std::vector<double> > &b)
{
    ...
    for(int i=0;i<a.size();i++)
        for(int j=0;j<a.size();j++)
            if((absolute(a[i][j])-absolute(b[i][j]))>thres) return false;
    return true;
}
...
bool equals(const sparse_matrix &a, const sparse_matrix &b)
{
    ...
    for(int i=1;i<a.getSize()+1;i++)
        for(int j=1;j<a.getSize()+1;j++)
            if(absolute(a.at(i,j)-b.at(i,j))>thres)
            {
               // std::cout << a.at(i,j) << " != " << b.at(i,j) << std::endl;
                B=false;
            }
    return B;
}
